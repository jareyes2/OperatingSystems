...
//---->
   sem_wait(&global);  //semiphore to order wizards when creating
    
  /*Should be frozen*/
  while(self->status == 1){ 
	printf("Wizard %c%d in room (%d,%d) is frozen\n",  self->team, self->id, oldroom->x, oldroom->y);
        sem_post(&alternate);
        sem_wait(&global);
  }
...
//---->
	...
	/* Fill in */
//---->
      sem_wait(&newroom->room_lock); //locks room prior to moving into the room            
//---->	
        ...
        if (other == NULL)
	{
        ...
        /* Fill in */
//---->
//After moving into the room posted to get back to interface
      sem_post(&alternate);
//---->
        ...
        else
	{
	  ...
	  /* Fill in */
//---->
//after finished fighting or freezing wizard return to interface
     sem_post(&alternate);
//---->
	}
